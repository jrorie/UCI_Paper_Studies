...
for x in range(1, number_of_loops+1):
	...
	#history = model.train_on_batch(X_train, Y_train, epochs=number_of_epochs, batch_size=set_batch_size)
	#(loss, TOB_accuracy) = model.train_on_batch(X_train, Y_train)
	print "Fitting and making history"
	history = model.fit(X_train, Y_train, epochs=number_of_epochs, batch_size=set_batch_size)
