...
#numpy.save('/storage1/users/jtr6/UCI_paper_data_sample/1000_train.npy', dataset)
#numpy.save('/storage1/users/jtr6/UCI_paper_data_sample/1000_test.npy', testset)
...
plt.savefig('1000_accuracy_%s.png' % now)
plt.show()
plt.clf()
...
plt.xlabel('epoch')
plt.legend(['train', 'test'], loc='upper left')
plt.savefig('1000_loss_%s.png' % now)
plt.show()
plt.clf()
...
now = time.strftime("%X")
plt.savefig('1000_predictions_%s.png' % now)
plt.show()
...
plt.show()
print('AUC: %f' % roc_auc)
plt.savefig('1000_ROC_%s.png' % now)
